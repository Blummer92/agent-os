...
def _split_plan(functions: list[dict[str, Any]], available: float, adapted_expected: float) -> dict[str, Any] | None:
    cumulative = 0.0
    ...
    total = adapted_expected
    first_period = min(cumulative, total)
    return {
        "split_after": split_after,
        "first_period_expected_minutes": first_period,
        "continuation_expected_minutes": max(0.0, total - first_period),
...
def plan_lesson_adaptation(
    ...
    if adapted["expected"] > available and packet["continuation_allowed"]:
        split_plan = _split_plan(packet["instructional_functions"], available, adapted["expected"])
