...
def _validate_observation_quality(value: object) -> dict[str, Any]:
    ...
    if "status" not in fields or value["status"] not in OBSERVATION_QUALITY_STATES:
    ...
    if any(code not in OBSERVATION_QUALITY_REASON_CODES for code in codes):
        raise ContractValidationError("handoff-unknown-field", "observation_quality reason_codes are not canonical LP14 codes")
