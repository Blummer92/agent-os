...
def _select_assets(assets: list[dict[str, Any]], include: bool) -> list[dict[str, Any]]:
    ...
    for item in assets:
        if item.get("canonical_unit_relation") is not True: continue
        selected.append({key: copy.deepcopy(value) for key, value in item.items() if key != "canonical_unit_relation"})
