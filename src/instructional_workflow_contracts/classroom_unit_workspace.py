...
def validate_classroom_unit_workspace(value: object) -> ValidationResult:
    ...
    try:
        ...
        if type(payload) is not dict:
            raise ContractValidationError("destination-workspace-invalid", "workspace must be an object")
        if payload.get("contract_version") != CONTRACT_ID:
            raise ContractValidationError("destination-workspace-version-invalid", "unsupported workspace contract")
        ...
        if root is None or root["state"] in {"missing", "ambiguous"} or root["drive_folder_id"] is None:
            raise ContractValidationError(
                "destination-workspace-root-invalid", "unit-root must identify one exact Drive folder"
            )
        for item in bindings:
            if item["role"] != "unit-root" and item["drive_folder_id"] is not None:
                ...
                if parent is not None and parent != root["drive_folder_id"]:
                    raise ContractValidationError(
                        "destination-workspace-parent-contradiction",
        ...
        if type(normalized) is not dict or canonical_size(normalized) > MAX_RESULT_BYTES:
            raise ContractValidationError("destination-workspace-oversized", "workspace exceeds result-size bound")
        ...
        if status is ValidationStatus.MANUAL_REVIEW_REQUIRED:
            reasons = ("destination-workspace-role-review-required",)
        ...
        return invalid_result("destination-workspace-invalid", sanitize_detail(str(exc)))


def _validate_bindings(value: object) -> list[dict[str, Any]]:
    if type(value) is not list or not value:
        raise ContractValidationError("destination-workspace-bindings-invalid", "role_bindings must be a non-empty list")
    result: list[dict[str, Any]] = []
    seen_roles: set[str] = set()
    for raw in value:
        if type(raw) is not dict:
            raise ContractValidationError("destination-workspace-binding-invalid", "role binding must be an object")
        role = raw.get("role")
        state = raw.get("state")
        if role not in WORKSPACE_ROLES:
            raise ContractValidationError("destination-workspace-role-unsupported", "role is unsupported")
        if role in seen_roles:
            raise ContractValidationError("destination-workspace-role-duplicate", "semantic role is bound more than once")
        seen_roles.add(role)
        if state not in BINDING_STATES:
            raise ContractValidationError("destination-workspace-binding-state-invalid", "binding state is unsupported")
        ...
        if state in {"resolved", "unverified", "stale"} and folder_id is None:
            raise ContractValidationError("destination-workspace-folder-id-missing", "binding state requires exact Drive folder ID")
        if state in {"missing", "ambiguous"} and folder_id is not None:
            raise ContractValidationError("destination-workspace-binding-contradiction", "missing/ambiguous role cannot claim exact folder identity")
        if state == "resolved" and last_verified is None:
            raise ContractValidationError("destination-workspace-currentness-missing", "resolved binding requires verification evidence")
        result.append(
            {
                "role": role,
                "state": state,
...
def _validate_binding_relationships(bindings: list[dict[str, Any]]) -> None:
    ...
    for item in bindings:
        ...
        if other is not None and other != item["role"]:
            raise ContractValidationError(
                "destination-workspace-folder-identity-conflict",
...
def _optional_text(value: object, field: str) -> str | None:
    ...
    if type(value) is not str or not value.strip() or len(value) > 512:
        raise ContractValidationError("destination-workspace-text-invalid", f"{field} is invalid")
    return value.strip()
