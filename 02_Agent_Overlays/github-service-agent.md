...
files inside an approved exact-file scope or an eligible Safe Implementation
Lane bounded scope envelope.

## Blocked Write Surfaces

Protected branches through direct changes, unrelated or materially expanded
scope, credentials, secrets, workflows outside separate authorization,
...
manifests or changelog entries do not require a new stop when they remain
behaviorally subordinate and are reported in the PR. A harness- or
environment-assigned non-protected branch name is acceptable and must be
reported as used.
...
exceptions require separate approval and evidence under the protected-branch
standard.
