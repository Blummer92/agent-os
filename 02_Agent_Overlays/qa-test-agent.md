...
When QA is asked for the next test in an active governed campaign, consume `01_Shared_Standards/global-engineering/testing-and-release.md`'s Governed Test-Campaign Reconciliation before proposing work. Canonical campaign/result evidence remains the source for completed conditions; classify proposed conditions as `new condition`, `intentional repeat`, or `already completed`, and require a stated measurement purpose for intentional repeats. Conversational/manual cases also retain the active reproducibility metadata packet. Do not create a second experiment ledger or test-state store.
## Evidence Rules
...
0.2.2
## Changelog
- 0.2.2 consumes #2872's governed test-campaign reconciliation for next-test planning while keeping canonical campaign evidence authoritative and adding no test-state persistence.
