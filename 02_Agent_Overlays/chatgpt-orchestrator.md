...
This section owns only the bounded route for requests that canonical request/context evidence resolves to an existing Picture Perfect / PPUX tutorial prompt artifact. It does not create a second request interpreter, phrase matcher, image-intent framework, tutorial model, execution path, or benchmark ledger.
...
- Numbered tutorial identities require canonical evidence. If a requested `Tutorial N` identity or workflow cannot be resolved, do not label generated prompts or benchmark output as `Tutorial N`; the unknown or unresolved workflow remains blocking until canonical evidence is retrieved or supplied. Adjacent photography, design, hierarchy, or other content-domain context cannot promote an exploratory prompt into a numbered tutorial.
...
- Preserve benchmark provenance as separate facts rather than collapsing prompt authoring, orchestration, execution, and provider output into a single `runner` label. When benchmark evidence is reported, identify `prompt_author`, `execution_surface`, `image_provider/model`, and `orchestrator` when known.
- `runner-generated` or `runner-orchestrated` claims require evidence that the governed runner actually performed that generation/orchestration step. If ChatGPT manually authors prompt text and the repository owner copies it into an external runner, report `manual ChatGPT chat authoring` and `user-operated external runner`; do not imply that Agent OS runner prompt generation or orchestration was tested.
- Manual prompt experiments cannot satisfy governed-runner acceptance criteria. If a runner was requested but has not actually been invoked, state that the current output is manual preparation for runner execution or fail visibly when the governed path itself is the requested test.
...
## Version
0.3.7
Compatibility lineage: 0.3.6, 0.3.5, 0.3.4, 0.3.3, 0.3.2, 0.3.1

## Changelog
- 0.3.7 hardens Picture Perfect tutorial/benchmark grounding for #2423/#2424: numbered tutorial identity requires canonical evidence, unresolved workflows cannot be promoted from adjacent content-domain context, and benchmark reporting separates manual prompt authoring, execution surface, provider/model, and actual runner orchestration without creating a second tutorial registry, runner, or benchmark ledger.
