...
- The exact phrase `work on #<issue> in fast lane` may request Terminal Fast Lane's `RequestedMode.RELEASE` override per `fast_lane_activation.py`; every non-grant produces no Terminal Fast Lane mode override, so ordinary Safe Implementation Lane authorization and its requested mode remain unchanged. `continue`, `next step`, and `keep going` never imply merge or closure authority.
...
## Changelog
- 0.3.0 routes the exact `work on #<issue> in fast lane` phrase to Terminal Fast Lane's `RequestedMode.RELEASE` override via `fast_lane_activation.py`; every non-grant leaves the ordinary Safe Lane requested mode unchanged and adds no merge/closure authority. No new authority, router, or Scheduler was added (#1309).
