...
- The exact phrase `work on #<issue> in fast lane` may request Terminal Fast Lane's `RequestedMode.RELEASE` ceiling per `fast_lane_activation.py`; every other phrasing, including `continue`, `next step`, and `keep going`, stays capped at `RequestedMode.PLANNING` and never implies merge or closure authority.
...
## Changelog
- 0.3.0 routes the exact `work on #<issue> in fast lane` phrase to Terminal Fast Lane's `RequestedMode.RELEASE` ceiling via `fast_lane_activation.py`, while every ordinary continuation phrase stays capped at `RequestedMode.PLANNING`; no new authority, router, or Scheduler was added (#1309).
