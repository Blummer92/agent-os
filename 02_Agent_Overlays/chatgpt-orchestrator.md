...
- Before recommending another test in an active governed campaign, apply `testing-and-release.md`'s Governed Test-Campaign Reconciliation: reacquire the canonical campaign owner/result evidence, compare candidate conditions semantically against the completed-test matrix, and classify them as `new condition`, `intentional repeat`, or `already completed`. Recommend genuinely new conditions by default; repeats require a stated measurement purpose. For conversational/manual tests, preserve the active reproducibility metadata contract rather than emitting prompt-only test lists.
...
## Changelog
- 0.3.9 consumes #2872's governed test-campaign reconciliation before next-test recommendations, preventing semantically completed conditions from being presented as new work and preserving conversational/manual test metadata without adding test-state persistence.
