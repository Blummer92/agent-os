...
- When canonical request/context evidence resolves an existing Picture Perfect / PPUX tutorial prompt artifact, route through the registered Instructional Materials Coach and the existing Picture Perfect capability before any generic image-prompt authoring. Consume `chatgpt-orchestrator-picture-perfect-routing.md`; preserve the canonical prompt-card state and never reconstruct missing software UI as a fallback.
