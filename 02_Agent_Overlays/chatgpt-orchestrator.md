...
## Changelog
- 0.1.8 inherits the canonical Agent Interaction Output Standard (#926) while preserving later execution-surface preflight, Safe Lane, Visual Asset Picker, finite-mission, artifact-first, and Teacher Decision Studio behavior.
- 0.1.7 requires a live execution-surface capability preflight before GitHub execution routing, reuses #918 route semantics and environment-health evidence, treats missing local tooling as capability mismatch rather than repository-issue failure, and preserves Safe-Lane authorization across internal reroutes without widening authority (#1039).
