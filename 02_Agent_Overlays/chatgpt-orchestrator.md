...
- For every authorized Agent OS issue creation, route through the existing canonical connected issue-creation/managed-label contract before treating the operation as complete. Derive supported managed labels from canonical tiered issue metadata, preserve duplicate/pre-create admission, and treat any native/direct GitHub create response as provisional until the exact issue is read back and the planned managed labels have converged through the existing #1962 reconciler. Missing or partial managed labels are intermediate state, not terminal success; ambiguous canonical owner/readiness/type evidence fails closed/manual-review rather than being guessed. This applies to bug capture, testing/planning issues, implementation handoffs, focused successors, and self-defect creation, and adds no second label writer or authority system (#2752).
...
- When a current PR or authorized branch head changes during an unfinished repair, do not classify head movement alone as foreign-active work or a terminal blocker. Reacquire the exact PR, new head, base, authorization, scope, changed-path evidence, and canonical Scheduler lease evidence; consume the existing Safe Implementation Lane `HEAD_ADVANCED` / `ResumePlan` contract; and classify the concurrent change before deciding whether to stop. Continue automatically when the new head is the same authorized lineage and the concurrent change is non-conflicting, already satisfies the intended repair, or permits the bounded repair to be replayed on the new head. Never write against the stale head, force-replace the newer head, reuse stale exact-head validation, or assume compatibility. Same-file or same-invariant conflict, out-of-scope change, unknown lineage, active/ambiguous conflicting lease, or changed authorization remains fail-closed and item-local in a finite campaign; independently actionable later items continue.
...
- A successful subordinate GitHub mutation during an unfinished finite mission is also intermediate evidence, not a terminal result. This includes issue/PR comments, label mutations, handoff records, and evidence-persistence writes performed by the GitHub Service Agent. Treat the mutation response as provisional; read back the canonical mutated target to prove persistence/currentness, then reacquire the parent issue/PR/branch/head/CI checkpoint when those mutable facts control remaining work. Continue to the next admitted operation in the same lineage while authorization, source of truth, and bounded scope remain current. Do not emit the parent mission's final report merely because the subordinate write succeeded.
...
- Route unclear targets or changed authorization/source-of-truth/scope/material decisions to a stop report instead of guessing. A changed PR head by itself is not such a stop: first reacquire and classify it through the existing Safe Implementation Lane `HEAD_ADVANCED` and Scheduler lease/currentness contracts.
...
- Resolve the registered reusable capability `ppux-picture-perfect-prompt-projection` and its fixed #2525 operation identity before generic image-prompt authoring. Route through the registered Instructional Materials Coach and the existing Picture Perfect package at `08_Tooling/instructional-materials-coach/picture-perfect-coach/`; do not duplicate its prompt engine, Tutorial 0 fixture, capture binding, ImageIntent contract, transport, or source resolver.
- Invoke only the fixed governed operation `ppux-picture-perfect-prompt-projection` after canonical tutorial/source-handoff resolution. Consume only `picture-perfect-prompt-projection-result-v1` evidence from that route; policy prose, fixtures, and generic language generation are not substitutes for a missing result.
...
- If the requested tutorial or prompt artifact cannot be resolved from canonical context, return `source-unresolved` or manual review. If the #2525 operation/result is unavailable, non-executable, invalid, blocked, or ambiguous, surface the corresponding bounded failure state. Do not silently fall back to generic generation.
- Consume governed result fields without rewriting canonical state: preserve tutorial/source identity, source fingerprint, execution SHA/projection identity, prompt-card order, ready/blocked state, blocker reason codes and teacher-facing explanations, application identity, provider-neutral `portablePrompt`, and provenance. A fully blocked governed result is a successful routing outcome.
- A downstream provider/destination such as Canva remains separate from the canonical modeled tutorial application and must not mutate application identity.
...
- 0.3.7 registers and consumes the governed #2525 Picture Perfect prompt-projection operation for #2526: resolved existing PPUX tutorial prompt requests use `ppux-picture-perfect-prompt-projection`, consume `picture-perfect-prompt-projection-result-v1` without rewriting canonical state, preserve ready/blocked cards, provenance, application/provider separation, and fail closed to finite unavailable/manual-review outcomes instead of generic reconstruction.
