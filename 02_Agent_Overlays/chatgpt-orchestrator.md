...
- If the requested tutorial or prompt artifact cannot be resolved from canonical context, return `source-unresolved` or manual review. If the #2525 operation/result is unavailable, non-executable, invalid, blocked, or ambiguous, surface the corresponding bounded failure state and do not silently fall back to generic generation.
