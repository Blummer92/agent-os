...
def activate_agent_os_failed_repair(*, repository: str, issue_number: int, attempt_id: str, failed_hypothesis: str, result_summary: str, task_reference: str, ecosystem_hints: tuple[str, ...] = (), language_hints: tuple[str, ...] = (), library_hints: tuple[str, ...] = (), capability_keywords: tuple[str, ...] = (), target_path_hints: tuple[str, ...] = (), canonical_rule_refs: tuple[str, ...] = (), known_knowledge_refs: tuple[str, ...] = (), specialized_knowledge_required: bool | None = None, execute_read: Callable[[Mapping[str, Any]], Mapping[str, Any]] | None = None, repair_context: str = "failed-pr-repair") -> dict[str, object]:
    ...
    return {"repository": repo, "issue_number": issue, "attempt_id": result.attempt.attempt_id, "retry_reentry_outcome": result.attempt.retry_reentry_outcome.value, "lesson_retrieval_status": result.lesson_result.lesson_retrieval_status.value, "selected_lesson_ids": list(result.lesson_result.selected_lesson_ids), "canonical_github_refs": list(result.lesson_result.canonical_github_refs), "mutation_admissible": result.boundary.mutation_admissible, "blocking_attempt_id": result.boundary.blocking_attempt_id, "reason_codes": list(result.boundary.reason_codes), "github_writes_authorized": False, "execution_authorized": False, "side_effects_performed": False}
