...
def _patch_nested_transports(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda value: asdict(value) | {"phase": value.phase.value},
    )
    monkeypatch.setattr(
        capsule_module,
        ...
        lambda value: FakePacket(
            **{
                **value,
                ...
                "stage_identities": tuple(tuple(x) for x in value["stage_identities"]),
    ...
    monkeypatch.setattr(
        ...
        lambda value: {
            "required_validation_command_ids": list(value.required_validation_command_ids)
        },
    )
    monkeypatch.setattr(
        capsule_module,
...
def _packet_and_approval(monkeypatch):
    _patch_nested_transports(monkeypatch)
    approval_id = "approval:" + "d" * 64
    revision = "approval-revision:" + "f" * 64
    packet = FakePacket(
        CandidatePacketPhase.EXECUTION_CANDIDATE,
        "Blummer92/agent-os",
        1978,
        "invocation-1978",
        "issue-1978-approval-custody",
        "main",
        "a" * 40,
        "b" * 40,
        "b" * 40,
        "c" * 40,
        (("approval-decision", f"{approval_id}@{revision}"),),
...
def _capsule(
    ...
    **overrides,
    ...
    packet = FakePacket(
        ...
        1799,
        "invocation-1799",
        "agent/1799-pre-publication-two-phase-envelope",
        "main",
        "a" * 40,
        "b" * 40,
        "b" * 40,
    ...
    values = dict(
        ...
        workspace_request_id="workspace-request:1799",
        invalidation_events=(),
        checkpoint_id=checkpoint_id,
        created_at="2026-09-03T14:00:00Z",
        expires_at="2026-09-03T15:00:00Z",
        evaluated_at="2026-09-03T14:10:00Z",
        phase=phase,
...
def _custody(monkeypatch, **overrides):
    packet, approval = _packet_and_approval(monkeypatch)
    values = dict(candidate_packet=packet, approval_record=approval)
    values.update(overrides)
    return capsule_module.build_approval_custody_evidence(**values)
...
def test_approval_custody_round_trip_is_minimal_deterministic_and_non_authorizing(monkeypatch):
    custody = _custody(monkeypatch)
    payload = capsule_module.serialize_approval_custody_evidence(custody)
    rebuilt = capsule_module.deserialize_approval_custody_evidence(payload)
    assert rebuilt == custody
    assert rebuilt.phase == capsule_module.APPROVAL_CUSTODY_PHASE
    assert rebuilt.schema_version == capsule_module.APPROVAL_CUSTODY_SCHEMA_VERSION
    assert rebuilt.capsule_id == capsule_module.approval_custody_evidence_id(rebuilt)
    assert rebuilt.execution_authorized is False
    assert rebuilt.publication_authorized is False
    assert rebuilt.github_writes_authorized is False
    ...
    for excluded in (
        ...
        assert excluded not in decoded


def test_approval_custody_rejects_wrong_phase_state_identity_and_binding(monkeypatch):
    packet, approval = _packet_and_approval(monkeypatch)
    with pytest.raises(ValueError, match="execution-candidate"):
        capsule_module.build_approval_custody_evidence(
            candidate_packet=replace(packet, phase=CandidatePacketPhase.APPROVAL_READY),
            approval_record=approval,
        )
    with pytest.raises(ValueError, match="approved"):
        capsule_module.build_approval_custody_evidence(
            candidate_packet=packet,
            approval_record=replace(approval, state=ApprovalState.REJECTED),
        )
    with pytest.raises(ValueError, match="approval identity"):
        capsule_module.build_approval_custody_evidence(
            candidate_packet=replace(
                packet,
                stage_identities=(("approval-decision", "approval:wrong@approval-revision:wrong"),),
            ),
            approval_record=approval,
        )
    with pytest.raises(ValueError, match="does not bind"):
        capsule_module.build_approval_custody_evidence(
            ...
            approval_record=replace(
                approval,
                binding=replace(approval.binding, tested_repository_sha="9" * 40),
            ),
        )


def test_approval_custody_rejects_incomplete_packet_and_payload_tamper(monkeypatch):
    packet, approval = _packet_and_approval(monkeypatch)
    with pytest.raises(ValueError, match="complete verified"):
        capsule_module.build_approval_custody_evidence(
            candidate_packet=replace(packet, evidence_completeness="partial"),
            approval_record=approval,
        )

    custody = capsule_module.build_approval_custody_evidence(
        candidate_packet=packet,
        approval_record=approval,
    )
    payload = json.loads(capsule_module.serialize_approval_custody_evidence(custody))
    payload["candidate_packet"]["tested_sha"] = "9" * 40
    with pytest.raises(ValueError):
        capsule_module.deserialize_approval_custody_evidence(json.dumps(payload))

    payload = json.loads(capsule_module.serialize_approval_custody_evidence(custody))
    payload["validation_bundle_id"] = "forbidden"
...
def test_approval_custody_store_is_idempotent_and_phase_loaders_are_separate(
    tmp_path,
    monkeypatch,
):
    custody = _custody(monkeypatch)
    first = store_module.append_pre_publication_evidence(tmp_path, custody)
    second = store_module.append_pre_publication_evidence(tmp_path, custody)
    assert first.already_present is False
    assert second.already_present is True
    assert first.path.parent.name == store_module.STORE_NAMESPACE
    assert (
        store_module.load_approval_custody_pre_publication_evidence(
            tmp_path, custody.capsule_id
        )
        == custody
    )
    with pytest.raises(CheckpointStoreIntegrityConflict, match="source-phase"):
        store_module.load_source_pre_publication_evidence(tmp_path, custody.capsule_id)
    with pytest.raises(CheckpointStoreIntegrityConflict, match="not publishable"):
        store_module.load_pre_publication_evidence(tmp_path, custody.capsule_id)
...
def test_source_phase_round_trip_is_deterministic_and_non_authorizing(monkeypatch):
    ...
    rebuilt = capsule_module.deserialize_pre_publication_evidence(
        capsule_module.serialize_pre_publication_evidence(source)
    )
...
def test_bind_source_rejects_checkpoint_binding_drift(monkeypatch, field, value):
    source = _capsule(monkeypatch)
    with pytest.raises(ValueError, match="does not bind"):
        capsule_module.bind_source_capsule_to_checkpoint(
            source, _checkpoint(source, **{field: value})
        )
...
def test_checkpoint_bound_store_requires_matching_durable_checkpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: replace(checkpoint, source_sha="9" * 40),
    )
    with pytest.raises(CheckpointStoreIntegrityConflict, match="does not bind"):
        store_module.append_pre_publication_evidence(
            tmp_path, replace(bound, capsule_id="")
        )
    assert first.path.parent.name == store_module.STORE_NAMESPACE
...
def test_no_second_store_or_external_authority_surface():
    ...
    assert "latest" not in store_source.casefold()
