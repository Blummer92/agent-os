...
def test_mission_completion_classifier_drives_remaining_same_lineage_work() -> None:
    result = classify_agent_os_mission_completion(
        repository="Blummer92/agent-os", issue_number=2189,
...
def test_mission_completion_classifier_is_terminal_only_after_pr_readback() -> None:
    result = classify_agent_os_mission_completion(
        repository="Blummer92/agent-os", issue_number=2189,
...
def test_failed_repair_admission_is_consumed_after_lesson_activation() -> None:
    result = admit_agent_os_failed_repair(
        activation_result={
            ...
            "selected_lesson_ids": ["63"],
            "mutation_admissible": True,
        },
        check_state="red", required_check_configuration_state="current",
        review_state="clear", branch_freshness="current", mergeability="mergeable",
    )
...
def test_capability_alternative_continues_same_lineage() -> None:
    result = classify_agent_os_continuation(
        repository="Blummer92/agent-os", issue_number=1213,
        operation_id="replace-exact-changelog", surface_outcome="selected-surface-unavailable",
        approved_alternative_capability="github-exact-blob-read", branch="agent/issue-1213",
        pull_request=1263, target_identity_reacquired=True,
        requires_exact_blob_identity=True, exact_blob_identity_reacquired=True,
    )
...
def test_ambiguous_prior_effect_requires_readback() -> None:
    result = classify_agent_os_continuation(
        repository="Blummer92/agent-os", issue_number=1213,
        operation_id="replace-exact-changelog", surface_outcome="selected-surface-unavailable",
        approved_alternative_capability="github-exact-blob-read", prior_effect="ambiguous",
    )
...
def test_desired_state_already_present_suppresses_duplicate_mutation() -> None:
    result = classify_agent_os_continuation(
        ...
        prior_effect="desired-state-already-present", target_identity_reacquired=True,
    )
    assert result["classification"] == "capability-alternative-available"
    assert result["continue_automatically"] is True
    assert result["mutation_permitted"] is False
...
def test_active_foreign_lease_never_creates_competing_execution() -> None:
    result = classify_agent_os_continuation(
        ...
        operation_id="governed-resume", surface_outcome="selected-surface-unavailable",
        approved_alternative_capability="governed-runner", active_foreign_lease=True,
    )
...
def test_cross_surface_transition_delegates_compatibility_until_proven() -> None:
    result = classify_agent_os_continuation(
        repository="Blummer92/agent-os", issue_number=1233,
        operation_id="governed-resume", surface_outcome="selected-surface-unavailable",
        approved_alternative_capability="governed-runner", target_identity_reacquired=True,
        runtime_surface_transition=True, evidence_compatibility_confirmed=False,
    )
...
def test_repeated_equivalent_transition_delegates_to_no_progress_owner() -> None:
    result = classify_agent_os_continuation(
        repository="Blummer92/agent-os", issue_number=1233,
        operation_id="governed-resume", surface_outcome="selected-surface-unavailable",
        approved_alternative_capability="governed-runner", equivalent_transition_repeated=True,
    )
...
def test_adjacent_lifecycles_are_not_absorbed(domain: str) -> None:
    with pytest.raises(ValueError, match="must not absorb"):
        classify_agent_os_continuation(
            repository="Blummer92/agent-os", issue_number=1233,
            operation_id="bounded-operation", surface_outcome="selected-surface-unavailable",
