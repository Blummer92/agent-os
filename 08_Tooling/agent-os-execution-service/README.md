...
## Execution-authorization source — Issues #1226 and #1986
`execution_authorization_source.py` reacquires explicit execution authorization from one injected read-only GitHub issue-comment snapshot. Only exact compact-JSON machine records authored by the current user-account repository owner are eligible; ordinary prose, non-owner comments, labels, authentication, candidate approval, workflow identity, Scheduler task status, and transport identity never authorize execution. The reader owns no GitHub client, mutation, retry, consumption/replay lock, Scheduler state, checkpoint, or persistent store.

Historical `agent-os-execution-authorization/v1` / schema `1.0` remains unchanged. It reconstructs the existing `ExecutionAuthorizationEvidence` for repository validation, with candidate-packet, invocation, operation, request, command-plan, repository/SHA, authorizer, time-window, revocation, and supersession bindings. Existing #757/#1218/provider consumers retain their exact v1 behavior and identity bytes. V1 and v2 markers are deliberately disjoint: the v1 parser ignores v2 records and the v2 parser ignores v1 records. Both reuse the same injected complete-snapshot trust, owner, ordering, source-completeness, duplicate-comment, expiry, revocation, and supersession principles. Neither record creates a Scheduler lease/replay lock or human approval, and neither performs Google/Scheduler/external I/O.

Issue #1986 adds the opt-in `agent-os-execution-authorization/v2` / schema `2.0` live-operation successor without changing v1. The v2 record binds exactly repository + issue, owner authorizer, the #1976 typed `subject_id`, typed approval ID/revision, typed projection ID, one pre-dispatch Scheduler logical attempt (`run_id`, `task_id`, non-negative `attempt_number`), the single finite operation `instructional-materials-live-operation`, `authorized_at`, `expires_at`, explicit `execution_authorized`, and a content-derived `execution-authorization:<sha256>` identity using a distinct v2 domain separator. It carries no command plan, SHA, Google API method, URL, credential, shell string, dynamic module path, or arbitrary provider operation. The v2 source subject for newest-record selection is the exact repository/issue + run/task/attempt + operation tuple. A newer matching false record revokes; a newer true identity supersedes an older expected identity; currentness requires `authorized_at <= evaluated_at < expires_at`; typed subject/approval/projection drift is stale; another run/task/attempt has no matching authorization and cannot replay the record. The source does not evaluate human approval applicability: #1976 remains the approval/currentness owner, and its projection remains non-authorizing. A future #1196 caller must first prove current #1976 approval/projection, then reacquire v2 against the immutable request attempt immediately before entering the C4A live-build boundary. The bridge-generated `ExecutionRequest.execution_id` is runtime trace evidence only and is not a preexisting authorization selector.
