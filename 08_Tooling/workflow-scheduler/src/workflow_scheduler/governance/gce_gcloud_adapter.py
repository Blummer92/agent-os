...
# Fixed trusted Python source. No value from issue-comment text is interpolated.
...
def spec(n):
 try:
  ...
  return None if s is None else {"origin":s.origin,"search_locations":list(s.submodule_search_locations or [])}
 except Exception as e:
  return {"error_class":type(e).__name__,"error":str(e)[:256]}
def meta(p):
 try:
  s=os.lstat(p)
  return {"path":p,"exists":True,"symlink":stat.S_ISLNK(s.st_mode),"owner":pwd.getpwuid(s.st_uid).pw_name,"group":grp.getgrgid(s.st_gid).gr_name,"mode":format(stat.S_IMODE(s.st_mode),"04o")}
...
base=spec("agent_os_execution_service"); sub=spec(M)
paths=[]
...
RUNTIME_INSPECTION_COMMAND = (
    f"{HOST_PYTHON} -c " + subprocess.list2cmdline([_RUNTIME_INSPECTION_SOURCE])
)
...
def _run(argv: Sequence[str], *, timeout: int = 60) -> subprocess.CompletedProcess[str]:
    return subprocess.run(tuple(argv), check=False, capture_output=True, text=True, timeout=timeout)
...
def _state(value: str) -> VmState:
    ...
    mapping = {"RUNNING": VmState.RUNNING, "TERMINATED": VmState.STOPPED, "STOPPED": VmState.STOPPED, "STAGING": VmState.STAGING, "STOPPING": VmState.STOPPING, "SUSPENDING": VmState.SUSPENDING}
...
class GcloudIapAdapter:
    """One-attempt gcloud adapter with no VM-stop or arbitrary-command authority."""
    ...
    def observe_state(self, resource: GceResourceTuple) -> VmState:
        result = _run(("gcloud","compute","instances","describe",resource.instance,*self._resource_args(resource),"--format=value(status)"))
        return _state(_require_ok(result, "observe instance state"))

    def start(self, resource: GceResourceTuple) -> bool:
        result = _run(("gcloud","compute","instances","start",resource.instance,*self._resource_args(resource),"--quiet"), timeout=120)
    ...
    def _ssh(self, resource: GceResourceTuple, command: str) -> subprocess.CompletedProcess[str]:
        return _run(("gcloud","compute","ssh",resource.instance,*self._resource_args(resource),"--tunnel-through-iap","--quiet","--command",command),timeout=180)

    def probe_ready(self, resource: GceResourceTuple) -> bool:
        return self._ssh(resource, f"test -x {FIXED_ENTRYPOINT}").returncode == 0

    def probe_discovery_ready(self, resource: GceResourceTuple) -> bool:
        return self._ssh(resource, DISCOVERY_PROBE_COMMAND).returncode == 0
...
def _policy()->OidcTrustPolicy:
    return OidcTrustPolicy(repository="Blummer92/agent-os",repository_owner="Blummer92",workflow_ref=WORKFLOW_REF,ref="refs/heads/main",audience=WIF_PROVIDER)
...
def execute_transport(ingress:IssueCommentIngressResult,*,claims:Mapping[str,object],adapter:GcloudIapAdapter)->dict[str,object]:
    if ingress.reason == "accepted-runtime-inspection-envelope":
        ...
        if not _policy().accepts(claims):
            return {"runtime_inspection":{"schema_version":"1.0","status":"blocked","reason_codes":["claims-rejected"],"project":PROJECT,"zone":ZONE,"instance":INSTANCE,"interpreter":HOST_PYTHON,"execution_authorized":False,"scheduler_invoked":False,"discovery_invoked":False,"resume_invoked":False,"side_effects_performed":False}}
        state=adapter.observe_state(RESOURCE)
        if state is not VmState.RUNNING:
            return {"runtime_inspection":{"schema_version":"1.0","status":"needs-decision","reason_codes":["host-not-running"],"project":PROJECT,"zone":ZONE,"instance":INSTANCE,"interpreter":HOST_PYTHON,"execution_authorized":False,"scheduler_invoked":False,"discovery_invoked":False,"resume_invoked":False,"side_effects_performed":False}}
    ...
    result=run_gce_control_path(request_id=binding.control_request_id,claims=claims,trust_policy=_policy(),resource=binding.resource,expected_resource=RESOURCE,handoff_id=binding.handoff_id,adapter=adapter,allow_shutdown=False)
...
def main(argv:list[str]|None=None)->int:
    ...
    parser.add_argument("--transport",type=Path,required=True);parser.add_argument("--output",type=Path,required=True);parser.add_argument("--repository",required=True);parser.add_argument("--repository-owner",required=True);parser.add_argument("--workflow-ref",required=True);parser.add_argument("--ref",required=True);parser.add_argument("--audience",required=True)
    ...
    evidence=execute_transport(_ingress_from_file(args.transport),claims=claims,adapter=GcloudIapAdapter())
    args.output.parent.mkdir(parents=True,exist_ok=True);args.output.write_text(json.dumps(evidence,sort_keys=True,separators=(",",":"))+"\n",encoding="utf-8");print(json.dumps(evidence,sort_keys=True));return 0
