...
def test_valid_request_returns_exact_bounded_deterministic_receipt():
    ...
    assert first["message"] == "Instructional Materials Coach command plan validated; no execution performed."
    assert first["output"]["command"] == [
        "imc-build", "build", "--content", "lessons/unit-1.yaml",
        "--slides-template", "slides-template_123", "--doc-template", "doc-template_123",
        "--target-folder", "folder_123", "--lessons-dir", "reports/lessons",
    ]
...
@pytest.mark.parametrize("field", ["contract_version", "operation", "content_path", "lessons_dir"])
def test_missing_and_blank_required_payload_fields_fail(field):
    payload = _payload()
    payload.pop(field)
    assert validate_instructional_materials_contract(_request(payload=payload))["status"] == "failure"
    payload = _payload()
    payload[field] = ""
    assert validate_instructional_materials_contract(_request(payload=payload))["status"] == "failure"


@pytest.mark.parametrize("key", ["token", "credentials", "client_secret", "oauth_path", "environment", "env", "command", "shell", "executable", "unexpected"])
...
@pytest.mark.parametrize("value", ["$(id)", "`id`", "x;y", "x>y", "x&&y", "x||y", "${HOME}", "x\ry", "x\ny", "x\x00y", "-rf", "/tmp/x", "../secret", "a/../secret", "C:/secret"])
...
@pytest.mark.parametrize("field", ["slides_template_id", "doc_template_id", "target_drive_folder_id"])
...
def test_valid_but_prohibited_states_are_blocked(request_changes, payload_changes, gate_changes, reason):
    payload = _payload()
    payload.update(payload_changes)
    payload["production_gates"].update(gate_changes)
    result = validate_instructional_materials_contract(_request(payload=payload, **request_changes))
    assert result == {
        "status": "blocked",
...
def test_subclassed_and_custom_values_fail_closed_without_custom_behavior():
    ...
    assert validate_instructional_materials_contract(_request(payload=payload))["status"] == "failure"
    assert validate_instructional_materials_contract(_request(payload=EvilDict(_payload())))["status"] == "failure"
    base = _request()
...
def test_malformed_types_and_bounds_fail_closed():
    ...
    assert validate_instructional_materials_contract(_request(payload=payload))["status"] == "failure"
    assert validate_instructional_materials_contract(_request(idempotency_key="a" * 257))["status"] == "failure"


def test_request_payload_and_context_are_not_mutated():
    request = _request(execution_context=ExecutionContext(approval_context={"note": "unchanged"}))
...
def test_wrong_contract_operation_owner_and_gate_shape_fail_closed():
    for field, value in (("contract_version", "v2"), ("operation", "other")):
        ...
        payload[field] = value
        assert validate_instructional_materials_contract(_request(payload=payload))["status"] == "failure"
    assert validate_instructional_materials_contract(_request(owner="other"))["status"] == "failure"
    payload = _payload()
