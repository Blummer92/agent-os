...
def test_different_handoffs_have_different_logical_trigger_identities()->None:
    assert admit(event(f"/agent-os resume {HANDOFF}")).logical_trigger_id_or_none!=admit(event(f"/agent-os resume executor-handoff:{'b'*64}")).logical_trigger_id_or_none
...
def test_cli_writes_bounded_canonical_json(tmp_path:Path)->None:
    e=tmp_path/"event.json";o=tmp_path/"out"/"transport.json";e.write_text(json.dumps(event(f"/agent-os resume {HANDOFF}")),encoding="utf-8");assert main(["--event",str(e),"--repository",REPOSITORY,"--allowed-actor",ACTOR,"--run-attempt","1","--output",str(o)])==0;p=json.loads(o.read_text());assert p["status"]=="accepted";assert p["execution_authorized"] is False;assert p["scheduler_invoked"] is False;assert p["side_effects_performed"] is False
...
def test_exact_first_publication_activation_is_bounded_and_non_authorizing()->None:
    r=admit(event(f"/agent-os activate-first-publication {SOURCE_CAPSULE}"));assert r.status=="accepted";assert r.reason=="accepted-first-publication-activation-envelope";assert r.source_capsule_id_or_none==SOURCE_CAPSULE;assert r.handoff_id_or_none is None;assert r.logical_trigger_id_or_none is not None;assert r.execution_authorized is False;assert r.scheduler_invoked is False;assert r.side_effects_performed is False
...
def test_duplicate_activation_comments_converge_but_capsule_change_does_not()->None:
    first=admit(event(f"/agent-os activate-first-publication {SOURCE_CAPSULE}"));dup_event=event(f"/agent-os activate-first-publication {SOURCE_CAPSULE}");dup_event["comment"]["id"]=9982;duplicate=admit(dup_event);changed=admit(event(f"/agent-os activate-first-publication pre-publication-evidence:{'d'*64}"));assert first.logical_trigger_id_or_none==duplicate.logical_trigger_id_or_none;assert first.logical_trigger_id_or_none!=changed.logical_trigger_id_or_none

def test_exact_dev_validation_trigger_is_accepted_but_non_authorizing()->None:
    r=admit(event(DEV_TRIGGER));assert r.status=="accepted";assert r.reason=="accepted-dev-validation-envelope";assert r.dev_validation_branch_or_none=="agent/1271-validation-profile-path-coverage";assert r.dev_validation_sha_or_none==DEV_SHA;assert r.dev_validation_id_or_none=="remote-validation-suite";assert r.handoff_id_or_none is None;assert r.logical_trigger_id_or_none is not None;assert r.execution_authorized is False;assert r.scheduler_invoked is False;assert r.side_effects_performed is False
...
def test_duplicate_dev_validation_comments_share_identity_but_sha_change_does_not()->None:
    first=admit(event(DEV_TRIGGER));p=event(DEV_TRIGGER);p["comment"]["id"]=9982;duplicate=admit(p);changed=admit(event(DEV_TRIGGER.replace(DEV_SHA,"c"*40)));assert first.logical_trigger_id_or_none==duplicate.logical_trigger_id_or_none;assert first.logical_trigger_id_or_none!=changed.logical_trigger_id_or_none
