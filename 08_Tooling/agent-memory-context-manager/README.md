...
- Within those processing limits, the source fingerprint binds the complete packet through canonical type-tagged UTF-8 data and the summary fingerprint binds the complete rendered contract. A structurally valid packet exceeding a renderer limit receives a deterministic `unverifiable` resource result and no trusted source fingerprint; renderer limits do not redefine #265 packet validity. Fingerprints are deterministic equality checks, not signatures or authorization.
