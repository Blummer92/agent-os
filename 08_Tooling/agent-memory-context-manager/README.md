...
- `packet_summary.summarize_handoff_packet(packet, evidence=...)` renders every canonical field in fixed order, reuses #265's validator as sole packet-validity authority, preserves truthful nullable branch/PR values, reports truncation, and keeps safety-critical sections visible.
- Rendering is bounded: caller-controlled strings are escaped and truncated, list limits are capped, compute-limit entries are capped, unsupported value types use fixed placeholders, and unsafe packet or evidence values fail closed instead of leaking object text or exceptions.
...
- The source fingerprint binds the complete packet through canonical type-tagged UTF-8 data. The summary fingerprint binds the rendered core, effective list limit, producer, schema and renderer identities, supplied and actual source fingerprints, provenance, trust result, and fixed non-authorization notice. Fingerprints are deterministic equality checks, not signatures or authorization.
