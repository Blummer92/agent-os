...
def test_main_visual_gap_blocks_before_credentials(monkeypatch, tmp_path, capsys):
    ...
    lessons_dir = tmp_path / "lessons"
    with patch("instructional_materials_coach.cli.get_credentials") as credentials:
        exit_code = cli.main(_base_build_args(lesson_file, requirement_file) + ["--visual-source-revision", "visual-library-snapshot-v2", "--lessons-dir", str(lessons_dir)])
    assert exit_code == 1
    credentials.assert_not_called()
...
def test_main_complete_visual_plan_blocks_before_credentials_and_preserves_selected_asset_id(monkeypatch, tmp_path, capsys):
    from instructional_materials_coach import cli
    monkeypatch.setenv("ALLOW_WRITE", "true")
    lesson_file = _lesson_file(tmp_path)
    requirement_file = _write_json(tmp_path, "visual-requirement.json", _fixture("valid_material_requirement_v2.json"))
    ...
    with patch("instructional_materials_coach.cli.get_credentials") as credentials, patch("instructional_materials_coach.cli.build_live_materials") as live:
        exit_code = cli.main(_base_build_args(lesson_file, requirement_file) + ["--artifact-manifests", str(manifests_file), "--visual-candidates", str(candidates_file), "--visual-source-revision", "visual-library-snapshot-v2", "--lessons-dir", str(lessons_dir)])
    assert exit_code == 1
    credentials.assert_not_called()
    live.assert_not_called()
    captured = capsys.readouterr()
    assert "Governed visual placement unavailable" in captured.err
    ...
    assert "selected_asset_ids=asset-1" in record["what_happened"]
