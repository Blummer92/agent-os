...
def recommend_route(
    *,
    ...
    if duplicate_disposition in {"EXACT_EXISTING", "NORMALIZED_EXISTING"}:
        ...
        return _plan(
            intake_reference, duplicate_disposition, existing_identity, observation, destination,
            RoutingState.REVIEW_REQUIRED, (TeacherAction.USE_EXISTING, TeacherAction.REVIEW),
            ("routing-existing-asset",),
        )
    if duplicate_disposition in {"MANUAL_REVIEW_REQUIRED", "INVALID"}:
        return _plan(
            intake_reference, duplicate_disposition, existing_identity, observation, destination,
            RoutingState.REVIEW_REQUIRED, (TeacherAction.REVIEW, TeacherAction.SKIP),
            ("routing-duplicate-review-required",),
        )
    if duplicate_disposition not in {"NO_DUPLICATE_FOUND", "POSSIBLE_NEAR_DUPLICATE"}:
    ...
    if observation.privacy_or_provenance_concern:
        return _plan(
            intake_reference, duplicate_disposition, existing_identity, observation, destination,
            RoutingState.REVIEW_REQUIRED, (TeacherAction.REVIEW, TeacherAction.SKIP),
            ("routing-source-review-required",),
        )
    if observation.ambiguous or observation.canonical_unit is None or observation.instructional_role is None:
        return _plan(
            intake_reference, duplicate_disposition, existing_identity, observation, destination,
            RoutingState.AMBIGUOUS, (TeacherAction.CHANGE, TeacherAction.SKIP),
            ("routing-semantic-ambiguous",),
        )
    if not destination.current or destination.drive_destination_ref is None or destination.notion_destination_ref is None:
        return _plan(
            intake_reference, duplicate_disposition, existing_identity, observation, destination,
            RoutingState.DESTINATION_STALE, (TeacherAction.CHANGE, TeacherAction.SKIP),
            ("routing-destination-unavailable",),
        )
    actions = (TeacherAction.ADD, TeacherAction.CHANGE, TeacherAction.SKIP)
    if duplicate_disposition == "POSSIBLE_NEAR_DUPLICATE":
        actions = (TeacherAction.USE_EXISTING, TeacherAction.CHANGE, TeacherAction.REVIEW)
    return _plan(
        intake_reference, duplicate_disposition, existing_identity, observation, destination,
        RoutingState.RECOMMENDED, actions, ("routing-recommendation-ready",),
    )


def apply_teacher_action(
    plan: AssetRoutingPlan,
    action: TeacherAction,
    *,
    correction: SemanticObservation | None = None,
) -> AssetRoutingPlan:
    ...
    if action is TeacherAction.CHANGE:
        ...
        return replace(
            ...
            canonical_unit=correction.canonical_unit,
            instructional_role=correction.instructional_role,
            semantic_provenance="teacher-confirmed",
            prior_advisory_concept=plan.concept,
            state=RoutingState.RECOMMENDED,
            allowed_actions=(TeacherAction.ADD, TeacherAction.CHANGE, TeacherAction.SKIP),
            teacher_confirmed=True,
            reason_codes=("routing-teacher-correction",),
        )
...
def _plan(intake_reference, duplicate_disposition, existing_identity, observation, destination, state, actions, reasons):
    return AssetRoutingPlan(
        ...
        instructional_role=observation.instructional_role,
        drive_destination_ref=destination.drive_destination_ref,
        notion_destination_ref=destination.notion_destination_ref,
        semantic_provenance=observation.provenance,
        prior_advisory_concept=None,
        state=state,
...
def _review_plan(intake_reference, duplicate_disposition, existing_identity, reason):
    ...
    observation = SemanticObservation("review-required", None, None)
    destination = DestinationContext(None, None, False)
    return _plan(safe_reference, str(duplicate_disposition)[:MAX_TEXT], existing_identity, observation, destination, RoutingState.REVIEW_REQUIRED, (TeacherAction.REVIEW, TeacherAction.SKIP), (reason,))


def _valid_observation(value: SemanticObservation) -> bool:
    return (
        type(value) is SemanticObservation
        and _valid_text(value.concept, MAX_TEXT)
        and (value.canonical_unit is None or _valid_text(value.canonical_unit, MAX_TEXT))
        and (value.instructional_role is None or _valid_text(value.instructional_role, MAX_TEXT))
        and _valid_text(value.provenance, MAX_TEXT)
        and type(value.ambiguous) is bool
        and type(value.privacy_or_provenance_concern) is bool
    )


def _valid_destination(value: DestinationContext) -> bool:
    return (
        type(value) is DestinationContext
        and (value.drive_destination_ref is None or _valid_text(value.drive_destination_ref, MAX_REFERENCE))
        and (value.notion_destination_ref is None or _valid_text(value.notion_destination_ref, MAX_REFERENCE))
        and type(value.current) is bool
    )
