...
**Execution Checkpoint Contract** (`scripts/agent_os_execution_checkpoint/`, #895, design approved in #858) is the pure-local checkpoint record, content-addressed append-only storage, and resume planner; every authority field stays false. Its canonical source location and ownership are unchanged by #1300 (AOS-GCE2C); that issue only made the same files installable by declaring them in the `workflow-scheduler` distribution, so exactly one implementation of the descriptor loader continues to exist.
