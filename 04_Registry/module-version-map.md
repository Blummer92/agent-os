...
**Workflow Scheduler**, **Agent Memory & Context Budget Manager**, **IA4D-to-Scheduler Handoff Contract**, and **Agent OS Execution Service** extended descriptions live in `04_Registry/module-version-map-details.md`. Workflow Scheduler moved to `0.10.0` under #758 for the additive host-local concurrency-1 lease adapter, moved to `0.11.0` under #759 (WSC-AUTO1C) for the additive, opt-in Linux cgroup v2 process-tree containment on the POSIX process adapter, moved to `0.12.0` under #760 (WSC-AUTO1D) for the additive complete workspace-state and changed-path evidence model, and moves to `0.13.0` under #762 (WSC-AUTO1F) for binding the #758/#759 seams into the concrete runtime composition and adding the additive #760 workspace-state capture wrapper (see `CHANGELOG.md`); Agent OS Execution Service moves from `0.5.0` to `0.6.0` under #762 for the additive end-to-end authorized-validation entrypoint composing #757/#758/#759/#760/#761 (see `CHANGELOG.md`).
