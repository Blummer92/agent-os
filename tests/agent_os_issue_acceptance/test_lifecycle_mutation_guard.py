...
def snapshot(**changes):
    data = dict(
        ...
        observed_revision="observation-1",
    )
    data.update(changes)
...
def authorization(**changes):
    data = dict(
        ...
        issue_number=768,
        pull_request_number=800,
        authorized_mutations=("mark-ready",),
        ...
        observed_at_revision="observation-1",
        state="authorized",
...
@pytest.mark.parametrize("mutation", ["mark-ready", "mark-draft", "merge"])
def test_valid_admissions(mutation):
    auth = authorization(authorized_mutations=(mutation,))
    result = evaluate_lifecycle_mutation(auth, snapshot(), mutation)
...
def test_merge_does_not_authorize_close():
    auth = authorization(authorized_mutations=("merge",))
    result = evaluate_lifecycle_mutation(auth, snapshot(), "close-issue")
    assert "authorization-mutation-not-permitted" in result.reason_codes
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"issue_number": 769}, "identity-issue-mismatch"),
        ({"pull_request_number": 801}, "identity-pull-request-mismatch"),
        ({"source_head": "c" * 40}, "pull-request-head-changed"),
        ({"base_head": "d" * 40}, "pull-request-base-changed"),
        ({"pr_state": "ready"}, "pull-request-ready-state-changed"),
        ({"merged": True}, "pull-request-merged-state-changed"),
        ({"issue_state": "closed"}, "issue-state-changed"),
        ({"review_state": "blocked"}, "review-state-changed"),
        ({"unresolved_threads": 1}, "review-thread-state-changed"),
        ({"lifecycle_labels": ("status:blocked",)}, "lifecycle-label-state-changed"),
        ({"observed_revision": "observation-2"}, "authorization-stale"),
    ],
)
...
def test_content_identity_changes_when_governed_input_changes():
    first = snapshot()
    second = snapshot(lifecycle_labels=("status:ready", "type:implementation"))
