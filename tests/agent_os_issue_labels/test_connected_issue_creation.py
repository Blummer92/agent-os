...
def test_connected_create_is_not_terminal_until_readback_converges():
    provider = Provider()
    result = converge_connected_issue_creation(provider, "Blummer92/agent-os", 2144, issue_form_path=FORM, label_map_path=MAP, label_write_authorized=True)
...
def test_missing_write_authority_cannot_claim_terminal_success():
    result = converge_connected_issue_creation(Provider(), "Blummer92/agent-os", 2144, issue_form_path=FORM, label_map_path=MAP, label_write_authorized=False)
