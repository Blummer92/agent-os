...
def test_one_focused_target_runs_before_structure(repo: Path) -> None:
    result = run(repo, "--focused", "tests/test_root.py", "--focused-maxfail", "1")
    assert result.returncode == 0
    assert result.stdout.index("==> focused: tests/test_root.py") < result.stdout.index("STRUCTURE_MARKER")
...
def test_multiple_focused_targets_preserve_declared_order(repo: Path) -> None:
    ...
    result = run(repo, "--focused", "tests/test_second.py", "--focused", "tests/test_root.py")
    assert result.returncode == 0
    assert result.stdout.index("focused: tests/test_second.py") < result.stdout.index("focused: tests/test_root.py")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("--focused", "tests/test_root.py", "--focused-maxfail", "0"), "between 1 and 100"),
        (("--focused", "tests/test_root.py", "--focused-maxfail", "-1"), "positive integer"),
        (("--focused", "tests/test_root.py", "--focused-maxfail", "101"), "between 1 and 100"),
        (("--focused", "tests/test_root.py", "--focused-maxfail", "999999999999999999999999"), "between 1 and 100"),
        (("--focused", "tests/test_root.py", "--focused-maxfail", "abc"), "positive integer"),
    ],
)
def test_invalid_arguments_fail_before_validation(repo: Path, args: tuple[str, ...], message: str) -> None:
...
def test_duplicate_focused_target_is_rejected(repo: Path) -> None:
    result = run(repo, "--focused", "tests/test_root.py", "--focused", "tests/./test_root.py")
    assert result.returncode == 2
...
def test_focused_failure_is_recorded_and_aggregate_continues(repo: Path) -> None:
    ...
    result = run(repo, "--focused", "tests/test_focused_fail.py", "--focused-maxfail", "1")
...
def test_governed_lanes_keep_one_canonical_runner_invocation() -> None:
    ...
    workflow = (root / ".github/workflows/agent-os-validation.yml").read_text(encoding="utf-8")
