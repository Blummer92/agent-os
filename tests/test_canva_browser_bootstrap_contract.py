...
SESSION = ROOT / "08_Tooling/agent-os-execution-service/scripts/agent-os-canva-browser-session"
INSTALLER = ROOT / "08_Tooling/agent-os-execution-service/scripts/install-canva-browser-bootstrap"
ADOBE_SESSION = ROOT / "08_Tooling/agent-os-execution-service/scripts/agent-os-adobe-browser-session"
TINKERCAD_SESSION = ROOT / "08_Tooling/agent-os-execution-service/scripts/agent-os-tinkercad-browser-session"
...
def test_fixed_canva_target_and_identity_are_not_selectable() -> None:
    ...
    assert '"browser_session_ref":"canva-default"' in normalized_session
    assert "START_URL=https://www.canva.com/" in session
    for forbidden in ("${1", "${2", "getopts", "read -r", "eval "):
        assert forbidden not in session


def test_canva_profile_identity_display_and_ports_are_isolated() -> None:
    session = _text(SESSION)
    adobe = _text(ADOBE_SESSION)
    tinkercad = _text(TINKERCAD_SESSION)

    assert "CAPTURE_USER=agent-os-canva-capture" in session
    assert "CAPTURE_HOME=/var/lib/agent-os/canva-capture-home" in session
    assert "PROFILE_DIR=$CAPTURE_HOME/.agent-os/browser-profiles/canva" in session
    assert "DISPLAY_ID=:99" in session
    assert "VNC_PORT=5903" in session
    assert "NOVNC_PORT=6082" in session
    assert "SESSION_TTL_SECONDS=1800" in session
    for other in (adobe, tinkercad):
...
def test_viewer_is_loopback_only_and_uses_qualified_x11vnc_flags() -> None:
    session = _text(SESSION)
...
def test_canva_exposes_no_remote_debugging_or_generic_browser_selector() -> None:
...
def test_dedicated_identity_and_root_owned_publication_are_fixed() -> None:
    ...
    assert "CAPTURE_USER=agent-os-canva-capture" in installer
    assert "INNER_TARGET=/usr/local/libexec/agent-os-canva-browser-session" in installer
    assert "OUTER_TARGET=/usr/local/libexec/agent-os-canva-browser-bootstrap" in installer
...
def test_cleanup_covers_transient_processes_and_preserves_canva_profile() -> None:
    session = _text(SESSION)
...
def test_lifecycle_evidence_is_bounded_non_secret_and_never_auth_ready() -> None:
    ...
    for required in (
        "agent-os-canva-browser-bootstrap-session",
        "canva-default",
