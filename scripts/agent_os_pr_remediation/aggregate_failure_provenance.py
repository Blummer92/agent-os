...
def classify_aggregate_failure(
    *, tested_sha: str, current_head_sha: str, main_sha: str | None,
    failure_fingerprint: str | None, same_failure_on_current_main: bool,
    environment_or_bootstrap_failure: bool, required_repository_invariant: bool,
    changed_contract_reaches_failure: bool, synthetic_merge_only_failure: bool,
    ...
    flags = (same_failure_on_current_main, environment_or_bootstrap_failure, required_repository_invariant, changed_contract_reaches_failure, synthetic_merge_only_failure)
    ...
    if tested != head and not synthetic_merge_only_failure:
        ...
        return AggregateFailureEvidence(AggregateFailureProvenance.AMBIGUOUS, tested, head, main, fingerprint, False, True, tuple(sorted(reasons)))
    if required_repository_invariant:
        reasons.add("required-repository-invariant-failed")
        return AggregateFailureEvidence(AggregateFailureProvenance.PR_ATTRIBUTABLE, tested, head, main, fingerprint, True, False, tuple(sorted(reasons)))
    ...
    if synthetic_merge_only_failure:
        ...
        return AggregateFailureEvidence(AggregateFailureProvenance.MERGE_CONTEXT, tested, head, main, fingerprint, True, False, tuple(sorted(reasons)))
    if environment_or_bootstrap_failure:
        if same_failure_on_current_main:
            ...
            return AggregateFailureEvidence(AggregateFailureProvenance.ENVIRONMENT, tested, head, main, fingerprint, False, False, tuple(sorted(reasons)))
        reasons.add("environment-or-bootstrap-failure")
        return AggregateFailureEvidence(AggregateFailureProvenance.ENVIRONMENT, tested, head, main, fingerprint, False, True, tuple(sorted(reasons)))
    if same_failure_on_current_main:
        ...
        return AggregateFailureEvidence(AggregateFailureProvenance.SHARED_BASELINE, tested, head, main, fingerprint, False, False, tuple(sorted(reasons)))
