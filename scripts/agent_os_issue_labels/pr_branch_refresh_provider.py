...
class GitHubPullRequestBranchRefreshBackingProvider(PullRequestBranchRefreshBackingProvider):
    ...
    def read_branch(self, repository: str, pr_number: int) -> PullRequestBranchSnapshot:
        try:
            ...
            changed_paths = tuple(sorted({str(item.filename) for item in pr.get_files() if getattr(item, "filename", None)}))
...
class ProductionPullRequestBranchRefreshProvider(PullRequestBranchRefreshProvider):
    ...
    def run_required_validation(self, repository: str, pr_number: int, *, head_sha: str, command_ids: tuple[str, ...]) -> BranchRefreshValidationResult:
        return self.backing.run_required_validation(repository, pr_number, head_sha=head_sha, command_ids=command_ids)
    ...
    def rebase_onto_main(self, repository: str, pr_number: int, *, expected_head_sha: str, expected_base_sha: str, current_main_sha: str) -> BranchRefreshMutationResult:
        ...
        blocker = _preparation_blocker(snapshot, repository=repository, pr_number=pr_number, expected_head_sha=expected_head_sha, expected_base_sha=expected_base_sha, current_main_sha=current_main_sha)
        ...
        merge_base_result = self.runner.run((self.git_binary, "merge-base", expected_head_sha, current_main_sha), cwd=self.repository_root, env=dict(self.environment))
        ...
        if use_merge_tree:
            ...
            head = self.runner.run((self.git_binary, "rev-parse", "--verify", "HEAD^{commit}"), cwd=self.repository_root, env=dict(self.environment))
        ...
        if update.status is ExpectedHeadBranchUpdateStatus.CONFIRMED:
            return BranchRefreshMutationResult(status="updated", old_head_sha=expected_head_sha, new_head_sha=proposed_head_sha, reason_code=update.reason)
    ...
    def _prepare_merge_shaped_candidate(self, *, expected_head_sha: str, current_main_sha: str, admitted_paths: tuple[str, ...]) -> str | BranchRefreshMutationResult:
        ...
        if merge_tree.return_code != 0:
            return _blocked(expected_head_sha, "reconciliation.semantic-conflict")
        ...
        main_epoch_result = self.runner.run((self.git_binary, "show", "-s", "--format=%ct", current_main_sha), cwd=self.repository_root, env=dict(self.environment))
        ...
        commit = self.runner.run((self.git_binary, "commit-tree", merged_tree_sha, "-p", current_main_sha, "-m", _TOPOLOGY_COMMIT_MESSAGE), cwd=self.repository_root, env=commit_env)
        ...
        checkout = self.runner.run((self.git_binary, "checkout", "--detach", proposed_head_sha), cwd=self.repository_root, env=dict(self.environment))
        ...
        scope = self.runner.run((self.git_binary, "diff", "--name-only", "--no-renames", current_main_sha, proposed_head_sha), cwd=self.repository_root, env=dict(self.environment))
...
def _preparation_blocker(snapshot: PullRequestBranchSnapshot, *, repository: str, pr_number: int, expected_head_sha: str, expected_base_sha: str, current_main_sha: str) -> str | None:
...
def _exact_head(observation: object) -> str | None:
    if not hasattr(observation, "succeeded") or not bool(getattr(observation, "succeeded")) or not isinstance(getattr(observation, "stdout", None), str):
        return None
...
def _blocked(old_head_sha: str, reason: str) -> BranchRefreshMutationResult:
    return BranchRefreshMutationResult(status="blocked", old_head_sha=old_head_sha, reason_code=reason)


def _ambiguous(old_head_sha: str, reason: str) -> BranchRefreshMutationResult:
    return BranchRefreshMutationResult(status="ambiguous", old_head_sha=old_head_sha, reason_code=reason)
