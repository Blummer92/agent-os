...
`plan_post_pr_lane(...)` preserves the selector's 1-3 selected lanes, emits the selector-first primary next issue, carries a compatible selected alternate, and reuses the canonical `ExecutorRoute` only when the audit recommendation confirms the selector primary. Recommendations that are absent, waiting for authorization/dependency, terminal, invalid, or deferred behind selector precedence fail closed to `human-decision`; unsupported alternates and audit-level `human-decision` do likewise. Source invariants are rechecked before planning so tampered semantic identities fail closed.
