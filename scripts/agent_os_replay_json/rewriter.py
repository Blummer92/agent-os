...
def _selector_evidence(steps: list[Any], source_indexes: tuple[int, ...]) -> tuple[str, ...]:
    selectors: list[str] = []
...
def _request_evidence_is_recorded(request_evidence: tuple[str, ...], recorded_evidence: tuple[str, ...]) -> bool:
...
class RewriteResult:
    ...
    def __post_init__(self) -> None:
        if self.semantic_equivalence not in SEMANTIC_EQUIVALENCE_STATES:
            raise ValueError(f"unsupported semantic equivalence: {self.semantic_equivalence}")
    ...
    for action_index, action in enumerate(actions):
        ...
        if action.kind in {"rename", "text_entry"} and len(source_indexes) > 1:
            ...
            if not change_steps:
                ...
                return RewriteResult(_isolated_recording(payload), tuple(operations), provenance, tuple(warnings), "rejected")
            ...
            operations.append(RewriteOperation(kind="replace-sequence", semantic_action_id=f"action-{action_index}", source_indexes=source_indexes, evidence=action.evidence, confidence="proven", output_indexes=(output_index,)))
            continue
        for source_index, step in zip(source_indexes, source_steps):
            ...
            operations.append(RewriteOperation(kind="keep", semantic_action_id=f"action-{action_index}", source_indexes=(source_index,), evidence=action.evidence, confidence="proven", output_indexes=(output_index,)))
    rewritten = _isolated_recording(payload)
...
def _rejected(payload: dict[str, Any], warning: str) -> RewriteResult:
    return RewriteResult(_isolated_recording(payload), (), {}, (warning,), "rejected")


def _unproven(payload: dict[str, Any], warning: str) -> RewriteResult:
    return RewriteResult(_isolated_recording(payload), (), {}, (warning,), "unproven")
...
def apply_request(payload: dict[str, Any], request: RewriteRequest) -> RewriteResult:
    ...
    selector_evidence = _selector_evidence(steps, action.source_indexes) if request.kind == "change-selector" else ()
    applicable_evidence = selector_evidence if request.kind == "change-selector" else action.evidence
    if not _request_evidence_is_recorded(request.evidence, applicable_evidence):
        warning = "request evidence is not present in recorded selector evidence" if request.kind == "change-selector" else "request evidence is not present in recorded evidence"
    ...
    if request.kind == "remove-noise":
        ...
        rewritten_steps = [deepcopy(step) for index, step in enumerate(steps) if index not in removed]
        provenance = {}
        output_index = 0
        ...
        rewritten = _isolated_recording(payload)
        rewritten["steps"] = rewritten_steps
        operation = RewriteOperation(kind="remove-noise", semantic_action_id=request.semantic_action_id, source_indexes=action.source_indexes, evidence=request.evidence or action.evidence, confidence="proven", output_indexes=())
    ...
    if not isinstance(source_step, dict):
        raise ValueError(f"steps[{source_index}] must be an object")
    rewritten_steps = [deepcopy(step) for step in steps]
    ...
    operation = RewriteOperation(kind="change-selector", semantic_action_id=request.semantic_action_id, source_indexes=action.source_indexes, evidence=request.evidence or selector_evidence, confidence="proven", output_indexes=(source_index,), changes_selector=True)
    return RewriteResult(rewritten, (operation,), provenance, (), "proven")
