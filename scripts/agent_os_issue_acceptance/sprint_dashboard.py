...
class SprintLaneEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "files_changed", tuple(sorted(set(self.files_changed))))
        object.__setattr__(self, "tests_run", tuple(sorted(set(self.tests_run))))
        object.__setattr__(self, "docs_updated", tuple(sorted(set(self.docs_updated))))
...
class SuppliedSprintEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.lanes) == 0 or len(self.lanes) > 3:
...
def recommended_merge_order(lanes: Iterable[SprintLaneEvidence]) -> tuple[int, ...]:
    rank = {
    ...
    return tuple(lane.issue for lane in sorted(lanes, key=lambda lane: (rank[lane.mode], lane.issue)))
...
def render_sprint_dashboard(repository: str, evidence: SuppliedSprintEvidence) -> str:
    ...
    for index, lane in enumerate(evidence.lanes, start=1):
        ...
        lines.append(
            f"| {index} | #{lane.issue} | {lane.mode.value} | {lane.compatibility.value} | {pr} | {reasons} |"
        )

    blockers = sorted({item for lane in evidence.lanes for item in lane.blockers})
    risks = sorted({item for lane in evidence.lanes for item in lane.risks})
    order = " → ".join(f"#{issue}" for issue in recommended_merge_order(evidence.lanes))
    lines.extend(
        [
            "",
            ...
            *( ["- none recorded"] if not risks else [] ),
        ]
    )
    if evidence.freshness != "current":
        lines.extend(
