...
_RESULT_STATUSES = frozenset({"complete", "stale", "blocked", "invalid", "needs-decision"})
...
class ApprovedExecutionProjection:
    ...
    def __post_init__(self) -> None:
        ...
        _matches(self.issueplan_current_state_evidence_id, _ISSUEPLAN_ID_RE, "issueplan_current_state_evidence_id")
        _sha256(self.repository_state_evidence_id, "repository_state_evidence_id")
        for name in ("handoff_digest", "graph_digest", "planning_result_digest", "source_snapshot_fingerprint", "scanner_result_fingerprint", "implementation_contract_fingerprint"):
            _sha256(getattr(self, name), name)
        for name in ("evaluated_repository_sha", "evaluator_commit_sha", "tested_repository_sha"):
            _sha40(getattr(self, name), name)
        for name in ("proposal_version", "repository", "base_branch", "approval_authorizer_id", "approval_decision_id"):
        ...
        if not isinstance(self.approval_revision_number, int) or isinstance(self.approval_revision_number, bool) or self.approval_revision_number < 2:
        ...
        if self.approval_supersedes_id is not None:
            _matches(self.approval_supersedes_id, _APPROVAL_ID_RE, "approval_supersedes_id")
        if self.repository_evidence_type not in {item.value for item in RepositoryEvidenceType}:
        ...
        if tuple(sorted(node for cohort in cohorts for node in cohort.node_ids)) != nodes:
        ...
        for name in ("allowed_files", "forbidden_paths", "required_tests"):
            object.__setattr__(self, name, _strings(getattr(self, name), name, allow_empty=True))
...
def build_approved_execution_projection(
    ...
    *,
    ...
    if planning_binding is not None and not isinstance(planning_binding, PlanningBindingEvidence):
        return _failure("invalid", ("projection.incomplete",), ("planning-binding:invalid-type",))
    if approval_record is None:
        return _failure("needs-decision", ("projection.lookup-failed",), ("approval-record:missing",))
    if not isinstance(issueplan_current_state_evidence, IssuePlanCurrentStateEvidence):
        return _failure("needs-decision", ("projection.incomplete",), ("issueplan-evidence:missing-or-invalid",))
    if not isinstance(repository_state_evidence, RepositoryStateEvidence):
        return _failure("needs-decision", ("projection.incomplete",), ("repository-evidence:missing-or-invalid",))
    if not isinstance(approval_applicability, ApprovalApplicabilityResult):
        return _failure("invalid", ("projection.incomplete",), ("approval-applicability:invalid-type",))
    try:
        ...
        reason = "version.unsupported" if getattr(approval_record, "schema_version", None) != "1.0" else "projection.incomplete"
        return _failure("invalid", (reason,), (f"verification:{exc}",))
    if approval_applicability != recomputed:
        return _failure("invalid", ("projection.incomplete",), ("approval-applicability:mismatch",))
    if approval_record.state != ApprovalState.APPROVED:
        lifecycle_reason = {ApprovalState.EXPIRED: "approval.expired", ApprovalState.INVALIDATED: "approval.invalidated", ApprovalState.SUPERSEDED: "approval.superseded"}.get(approval_record.state, "projection.incomplete")
        status = recomputed.status if recomputed.status != "applicable" else "blocked"
        return _failure(status, (*recomputed.reason_codes, lifecycle_reason), (f"approval-state:{approval_record.state.value}",))
    if recomputed.status != "applicable":
        reasons = recomputed.reason_codes or (("candidate.changed",) if recomputed.status == "stale" else ("projection.incomplete",))
    ...
    if repository_state_evidence.tested_sha is None:
        return _failure("needs-decision", ("projection.incomplete",), ("repository-tested-sha:missing",))
    try:
...
def serialize_approved_execution_projection(projection: ApprovedExecutionProjection) -> bytes:
    verified = _verified_projection(projection)
    return _canonical_bytes(_projection_payload(verified)) + b"\n"


def _verified_projection(projection: ApprovedExecutionProjection) -> ApprovedExecutionProjection:
    ...
    return ApprovedExecutionProjection(
        ...
        issueplan_current_state_evidence_id=projection.issueplan_current_state_evidence_id,
        ...
        implementation_contract_fingerprint=projection.implementation_contract_fingerprint,
...
def _semantic_payload(projection: ApprovedExecutionProjection) -> dict[str, Any]:
    ...
    for key in ("projection_id", "projected_at", "complete", "authoritative", "execution_authorized", "side_effects_performed"):
        payload.pop(key)
    return payload
...
def _projection_payload(projection: ApprovedExecutionProjection) -> dict[str, Any]:
    return {
        ...
        "cohort_summaries": [{"node_ids": list(item.node_ids), "classification": item.classification, "reason_codes": list(item.reason_codes)} for item in projection.cohort_summaries],
        "issueplan_current_state_evidence_id": projection.issueplan_current_state_evidence_id,
        "repository_state_evidence_id": projection.repository_state_evidence_id,
        "source_snapshot_fingerprint": projection.source_snapshot_fingerprint,
        "scanner_result_fingerprint": projection.scanner_result_fingerprint,
...
def _projection_id(payload: dict[str, Any]) -> str:
    return "approved-execution-projection:" + hashlib.sha256(_canonical_bytes(payload)).hexdigest()


def _failure(status: str, reasons: tuple[str, ...], details: tuple[str, ...]) -> ApprovedExecutionProjectionResult:
...
def _matches(value: str, pattern: re.Pattern[str], name: str) -> None:
...
def _cohorts(values: tuple[HandoffCohort, ...]) -> tuple[HandoffCohort, ...]:
    if not isinstance(values, tuple) or not values or not all(isinstance(item, HandoffCohort) for item in values):
        raise ValueError("cohort_summaries must contain HandoffCohort values")
    return values
...
def _reason_codes(values: tuple[str, ...]) -> tuple[str, ...]:
    ...
    if set(values) - APPROVAL_INVALIDATION_REASON_CODES:
