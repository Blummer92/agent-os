...
_NOT_APPLICABLE_BODY_PATTERNS = ("this issue is a roadmap only", "this issue is a tracker only", "coordination only; no implementation", "planning and gap-analysis issue only", "roadmap issue —", "roadmap issue -", "master epic / roadmap", "tracker only", "tracking issue", "planning and readiness only", "planning-only", "this roadmap does not authorize", "## current roadmap")
_TRACKING_TITLE_PATTERNS = ("roadmap", "tracking issue")
_TRACKING_BODY_EVIDENCE = ("## major sequence", "## current roadmap", "## child issues", "## existing child issues", "track a bounded", "track the complete", "this roadmap does not authorize", "does not authorize implementation")
...
def build_documentation_gap_report(records: Iterable[IssueScannerRecord], *, evaluator_revision: str) -> DocumentationGapReport:
    if not evaluator_revision.strip(): raise ValueError("evaluator_revision is required")
    grouped: dict[int, list[IssueScannerRecord]] = {}
    for record in records:
        if not isinstance(record, IssueScannerRecord): raise TypeError("records must contain IssueScannerRecord values")
        if record.state.lower() == "open": grouped.setdefault(record.issue_number, []).append(record)
    ...
    for number in sorted(grouped):
        ...
        else: rows.append(classify_documentation_gap(unique[0]))
...
def classify_documentation_gap(record: IssueScannerRecord) -> DocumentationGapRow:
    ...
    if _is_not_applicable(record, labels, body): return _row(record, DocumentationGapCategory.NOT_APPLICABLE, impact, (*canonical_codes, impact_code, "tracker-or-roadmap"), "No metadata backfill is recommended.")
    if not _is_candidate(labels, body): return _row(record, DocumentationGapCategory.NOT_APPLICABLE, impact, (*canonical_codes, impact_code, "not-implementation-candidate"), "No implementation-candidate evidence is present.")
    if check.status == Status.PASS: return _row(record, DocumentationGapCategory.ALREADY_COMPLIANT, impact, (*canonical_codes, "documentation-impact-present"), "No action is required.")
    if labels & _CLEANUP or _body_has(body, ("this issue is obsolete", "duplicate of #", "superseded by #")): return _row(record, DocumentationGapCategory.CLEANUP_CANDIDATE, impact, (*canonical_codes, impact_code, "possible-cleanup"), "Review for closure or supersession; do not mutate automatically.")
    if "status:blocked" in labels: return _row(record, DocumentationGapCategory.DEFER_BLOCKED, impact, (*canonical_codes, impact_code, "blocked-dependency"), "Re-evaluate after the recorded blocker clears.")
    if labels & _MANUAL_DECISION: return _row(record, DocumentationGapCategory.MANUAL_OWNER_DECISION, impact, (*canonical_codes, impact_code, "ambiguous-manual-review"), "Resolve the recorded decision before metadata backfill.")
    if "legacy-metadata-missing" in canonical_codes:
        missing = _missing_authority(body)
        if missing: return _row(record, DocumentationGapCategory.MANUAL_OWNER_DECISION, impact, (*canonical_codes, "documentation-impact-missing", "ambiguous-manual-review", *missing), "Resolve owner and source-of-truth evidence before backfill.")
...
def render_documentation_gap_report_text(report: DocumentationGapReport) -> str:
    m = report.metrics
    lines = ["Documentation-impact gap report", f"schema_version: {report.schema_version}", f"evaluator_revision: {report.evaluator_revision}", f"open_issue_count: {m.open_issue_count}", f"open_implementation_candidates: {m.open_implementation_candidates}", f"legacy_missing_documentation_impact: {m.legacy_missing_documentation_impact}", f"backfill_now_count: {m.backfill_now_count}", f"defer_blocked_count: {m.defer_blocked_count}", f"manual_owner_decision_count: {m.manual_owner_decision_count}", f"cleanup_candidate_count: {m.cleanup_candidate_count}", f"not_applicable_count: {m.not_applicable_count}", f"already_compliant_count: {m.already_compliant_count}", f"legacy_manual_review_rate: {m.legacy_manual_review_rate:.6f}", "rows:"]
...
def records_from_snapshot(payload: object) -> tuple[IssueScannerRecord, ...]:
    ...
    if not isinstance(raw, Sequence) or isinstance(raw, (str, bytes)): raise ValueError("snapshot must be a list or an object containing an issues list")
    required = {"issue_number", "title", "state", "body", "labels", "url", "created_at", "updated_at", "source_revision"}
    records: list[IssueScannerRecord] = []
    for item in raw:
        if not isinstance(item, Mapping): raise ValueError("each snapshot issue must be an object")
        missing = sorted(required - set(item))
        if missing: raise ValueError("snapshot issue missing field(s): " + ", ".join(missing))
        labels = item["labels"]
        if not isinstance(labels, Sequence) or isinstance(labels, (str, bytes)): raise ValueError("snapshot labels must be a list")
...
def _documentation_check(body: str) -> CheckResult:
    ...
    if len(matches) != 1: raise ValueError("canonical readiness result must contain one documentation impact check")
    return matches[0]
...
def _impact_value(check: CheckResult) -> str:
    for evidence in check.evidence:
        if evidence.startswith("impact="): return evidence.split("=", 1)[1]
...
def _is_not_applicable(record: IssueScannerRecord, labels: frozenset[str], body: str) -> bool:
    if labels & _NOT_APPLICABLE or _body_has(body, _NOT_APPLICABLE_BODY_PATTERNS): return True
...
def _missing_authority(body: str) -> tuple[str, ...]:
    ...
    missing: list[str] = []
    if not metadata.owner_agent and not any(value in headings for value in _OWNER_HEADINGS): missing.append("unclear-owner")
    if not metadata.source_of_truth and not any(value in headings for value in _SOURCE_HEADINGS): missing.append("unclear-source-of-truth")
    return tuple(missing)


def _authoritative_body_text(body: str) -> str:
    """Remove deterministic Markdown/example contexts before phrase classification."""
    ...
    for line in without_fences.splitlines():
        ...
        if not stripped or stripped.startswith(">"):
            continue
        if lowered.startswith(("example:", "example —", "example -", "for example:", "do not write ")):
            continue
...
def _body_has(body: str, phrases: tuple[str, ...]) -> bool:
    normalized = " ".join(_authoritative_body_text(body).lower().split())
...
def _metrics(rows: tuple[DocumentationGapRow, ...]) -> DocumentationGapMetrics:
    ...
    return DocumentationGapMetrics(open_issue_count=len(rows), open_implementation_candidates=len(candidates), legacy_missing_documentation_impact=missing, backfill_now_count=counts[DocumentationGapCategory.BACKFILL_NOW], defer_blocked_count=counts[DocumentationGapCategory.DEFER_BLOCKED], manual_owner_decision_count=counts[DocumentationGapCategory.MANUAL_OWNER_DECISION], cleanup_candidate_count=counts[DocumentationGapCategory.CLEANUP_CANDIDATE], not_applicable_count=counts[DocumentationGapCategory.NOT_APPLICABLE], already_compliant_count=counts[DocumentationGapCategory.ALREADY_COMPLIANT], legacy_manual_review_rate=missing / len(candidates) if candidates else 0.0)
