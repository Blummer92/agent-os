...
_ALLOWED_BINDINGS = frozenset({"repository", "issue_identity", "authorization_id", "scope_id", "execution_id", "candidate_id", "head_sha", "base_sha", "workspace_id", "executor_surface", "environment_id", "dependency_id", "lease_generation", "command_plan_id", "resume_lineage_id", "lifecycle_snapshot_id"})
_AUTHORITY_BINDINGS = frozenset({"repository", "issue_identity", "authorization_id", "scope_id"})
_MAX_RECORDS = 32
...
def _decision_id(*, context: CompatibilityContext, expected: ExpectedGeneration, records: tuple[CompatibilityEvidenceRecord, ...], outcome: CompatibilityOutcome, reasons: tuple[str, ...], reacquire_owners: tuple[str, ...]) -> str:
    payload = {"schema_version": EVIDENCE_COMPATIBILITY_SCHEMA_VERSION, "context": context.value, "expected": list(expected.bindings), "records": [{"evidence_id": record.evidence_id, "owner": record.owner, "bindings": list(record.bindings)} for record in records], "outcome": outcome.value, "reason_codes": list(reasons), "reacquire_owners": list(reacquire_owners), "authority_created": False, "side_effects_performed": False}
...
def evaluate_evidence_compatibility(*, context: CompatibilityContext, expected: ExpectedGeneration, records: tuple[CompatibilityEvidenceRecord, ...]) -> EvidenceCompatibilityDecision:
    ...
    if hard_reasons:
        ...
        reasons = ("compatible",)
        reacquire_owners = ()
    return EvidenceCompatibilityDecision(context=context, outcome=outcome, expected_bindings=expected.bindings, reason_codes=reasons, reacquire_owners=reacquire_owners, decision_id=_decision_id(context=context, expected=expected, records=canonical_records, outcome=outcome, reasons=reasons, reacquire_owners=reacquire_owners))


def evaluate_execution_dispatch_compatibility(*, expected: ExpectedGeneration, records: tuple[CompatibilityEvidenceRecord, ...]) -> EvidenceCompatibilityDecision:
    return evaluate_evidence_compatibility(context=CompatibilityContext.EXECUTION_DISPATCH, expected=expected, records=records)


def evaluate_ready_for_review_compatibility(*, expected: ExpectedGeneration, records: tuple[CompatibilityEvidenceRecord, ...]) -> EvidenceCompatibilityDecision:
    return evaluate_evidence_compatibility(context=CompatibilityContext.READY_FOR_REVIEW, expected=expected, records=records)
