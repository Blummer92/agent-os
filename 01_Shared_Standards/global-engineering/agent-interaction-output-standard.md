...
- Prompt/command delivery uses the smallest reusable context packet that executes safely. When the user explicitly asks for console, shell, GitHub CLI, or other executable commands, return executable commands rather than conversational prompts; keep connected sequences short and reuse variables/context where that reduces repeated typing. Default investigation commands to read-only unless mutation is separately requested and authorized. Do not repeat governance, source-of-truth, architecture, or repository boilerplate already available to the target unless material to the exact action.
