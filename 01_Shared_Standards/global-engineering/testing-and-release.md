...
- A focused pass is non-final evidence: treat it as `aggregate-pending`, not as final validation success. When focused checks are subsumed by an exact-head governed CI aggregate, that exact-head aggregate may provide both the required focused behavior evidence and the authoritative final aggregate evidence; do not require duplicate local execution solely to satisfy location.
