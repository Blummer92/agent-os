...
7. Before reusing any persisted reduction, retirement, refactor, or deletion-candidate inventory that claims to describe current repository paths, reconcile every candidate against current `main`. Treat the persisted inventory as historical evidence until that check completes. A candidate path absent from current `main` is `HISTORICAL-ONLY / already retired`: preserve its issue/PR/commit lineage as evidence, remove it from the active candidate population, and do not spend deeper consumer or retirement analysis on it. Rebuild the active population from only surviving current paths, then apply the existing contract-specific retirement or consolidation gate; absence alone never proves that a surviving path is safe to delete. Reuse existing currentness and retirement discipline rather than creating another scanner, registry, cache, currentness service, scheduler, or state store.
8. For bounded bug-work requests, build the implementation candidate population from currently open GitHub issues only, using `is:issue is:open` or an equivalent structured `state=open` filter at enumeration time, then reconcile that existing open bug backlog before fresh defect discovery. Closed issues, including closed issues with stale bug/readiness labels or entries in old handoffs/queues, must not enter candidate enumeration. After an open issue has been selected, closed issue/PR/commit history may be read only as supporting dependency, supersession, prior-implementation, or lineage evidence for that selected open issue; historical evidence never promotes a closed issue into actionable work. Exclude stale, duplicate, already-fixed, active-implementation, non-repository, blocked, or unauthorized open candidates; use eligible existing bugs first. Fresh defect discovery is allowed only when the user explicitly requests new bug discovery/logging, or when a distinct defect is discovered incidentally during authorized work and the governed process requires it to be recorded. Newly discovered or newly created bugs do not count toward a user-requested existing-backlog implementation count unless the user explicitly approves that substitution. If the reconciled eligible open backlog cannot satisfy the requested count, report the honest shortfall rather than creating issues merely to pad the count. In an active Agent OS backlog context, ambiguous wording such as `find 10 bugs` resolves to the existing canonical backlog; explicit wording such as `find 10 new bugs` authorizes fresh defect discovery. Bind each candidate disposition to the existing finite-mission cursor: already-fixed/completed, duplicate, blocked-item-local, separately-gated, external-owner/non-repository, and other candidate-local terminal dispositions are non-terminal for the parent batch and must immediately advance to the next independent open candidate without another user prompt or rebuilding the batch investigation. Stop later candidates only for a shared authorization, source-of-truth, bounded-scope, excluded-surface, capability, or material-decision blocker; otherwise continue until the requested count is worked or the reconciled open pool is exhausted and report the honest shortfall. Never fall back to closed issues as replacement candidates when open candidates are blocked, already implemented, or exhausted.
9. When a process/tooling bug is discovered and logged during an unfinished authorized mission, treat bug capture as subordinate bookkeeping rather than a terminal outcome. After the bounded bug write or alternate canonical persistence route completes, reacquire the parent issue/PR/branch/head checkpoint and continue the still-authorized parent mission without requiring another user prompt. Stop only when the discovered bug creates a genuine shared authorization, source-of-truth, bounded-scope, excluded-surface, capability, or material-decision blocker.
10. Treat every successful subordinate GitHub mutation during an unfinished finite mission — including an issue/PR comment, label mutation, handoff record, or evidence-persistence write — as provisional intermediate evidence, never as parent-mission completion by itself. Read back the canonical mutated target to prove persistence/currentness, then reacquire the parent issue/PR/branch/head/CI checkpoint when those mutable facts govern remaining work. Immediately before any Draft PR creation, reacquire the canonical issue and active primary-PR evidence and consume the existing primary-PR creation admission contract: create only when fresh evidence proves no active primary PR, reuse the single existing primary PR when one exists, and fail closed for multiple active primary claims. Continue the still-authorized parent mission automatically while in-scope work remains. Stop only at an existing governed terminal disposition or a genuine authorization, source-of-truth, bounded-scope, excluded-surface, capability, or material-decision blocker. This readback/continuation step never widens authority and never grants merge, issue-closure, workflow/protected-setting, credential, production, or external-write authority.
11. When a governed implementation or PR repair attempt remains red or otherwise fails, the next repair hypothesis or repository mutation is gated by `01_Shared_Standards/global-engineering/failed-repair-lesson-reentry.md`. Preserve the failed attempt, increase diagnostic resolution, satisfy that shared standard's existing retry-specific CKR6 re-entry and mutation-admission contract, then reacquire mutable GitHub state before continuing. Every newly failed attempt re-triggers this step; an earlier lesson outcome cannot satisfy a later failed attempt. Do not substitute the ordinary one-time coding preflight for this retry transition.
12. For eligible Safe Implementation Lane work, route registered-owner transitions internally while authorization, source of truth, and bounded scope remain applicable; do not require a user copy/paste handoff only because the owner changes.
13. Surface a handoff or decision only when authorization, source of truth, bounded scope, or a material decision changes.
